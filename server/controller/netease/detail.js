...
exports.getComicDetail = async function (url) {
    ...
    return await page.evaluate((targetUrl) => {
        ...
        let intro = document.querySelector('#intro').innerHTML.replace(/<br\s*\/?>/g, "\n") //获取介绍
        ...
        let loadMore = false
        if (document.querySelector('.js-flag.load-more')){
            loadMore = true
        }
        return {cover,title,author,category,intro,state,updateTime,data,loadMore}
    }, targetUrl)
}
